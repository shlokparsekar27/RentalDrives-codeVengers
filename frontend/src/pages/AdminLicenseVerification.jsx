...
import { FaCheck, FaExternalLinkAlt, FaIdCard, FaArrowLeft, FaCar, FaUserSlash, FaClock } from 'react-icons/fa';
...
function AdminLicenseVerification() {
    ...
    if (isLoading) return <div className="min-h-screen pt-32 text-center text-slate-500 font-mono bg-slate-50 dark:bg-[#020617] animate-pulse">LOADING DRIVER DATABANKS...</div>;
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                            ...
                            <h1 className="text-3xl md:text-4xl font-display font-bold text-white flex items-center gap-4">
                                Driver Verification
                                <span className="text-sm bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 px-3 py-1 rounded-full font-mono">
                                    Queue: {pendingTourists?.length || 0}
                                </span>
                            </h1>
                            <p className="text-slate-400 mt-2 max-w-lg text-lg">
                                Validate driving licenses for new renter accounts.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {pendingTourists && pendingTourists.length > 0 ? (
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {pendingTourists.map((tourist) => (
                                <div key={tourist.id} className="p-6 md:p-8 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                    <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-8">

                                        {/* Avatar / Icon */}
                                        <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-indigo-500 flex-shrink-0 border border-indigo-100 dark:border-indigo-800 shadow-inner group-hover:scale-105 transition-transform duration-300">
                                            <FaCar size={26} />
                                        </div>
                                        ...
                                        <div className="flex-grow">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h3 className="text-xl font-bold font-display text-slate-900 dark:text-white">{tourist.full_name}</h3>
                                                <Badge variant="warning" className="text-[10px] px-2 py-0.5">PENDING LICENSE</Badge>
                                            </div>
                                            <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                                                <span className="font-mono bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs">{tourist.email}</span>
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto mt-4 md:mt-0">
                                            <button
                                                ...
                                                className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-sm font-bold hover:bg-white dark:hover:bg-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 transition-all shadow-sm"
                                            >
                        ...
                        <div className="flex flex-col items-center justify-center h-[500px] opacity-50">
                            <div className="w-24 h-24 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-6 animate-pulse-slow">
                                <FaUserSlash className="text-slate-400 text-4xl" />
                            </div>
                            <h3 className="text-2xl font-bold font-display text-slate-900 dark:text-white">Queue Empty</h3>
                            <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">No pending driver license verifications.</p>
                        </div>
                    )}
                </div>
            </div>

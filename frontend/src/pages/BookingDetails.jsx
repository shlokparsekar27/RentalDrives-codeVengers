...
export default function BookingDetail() {
  ...
  const stepIndex =
   ...
   const token = localStorage.getItem("token"); // check it exists

const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bookings/${id}/cancel`, {
  method: "PATCH",
  headers: { 
    "Content-Type": "application/json",
    Authorization: `Bearer ${token}` // Must match authenticateToken format
  },
});
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow">
      ...
      <div className="mb-6">
        ...
        <p>
          <b>Status:</b>{" "}
          <span className={`px-2 py-1 rounded text-sm ${booking.status === "confirmed" ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}`}>
            {booking.status}
          </span>
        </p>
      </div>
      ...
      {steps.map((step, index) => {
        ...
        return (
          <div key={step} className="flex-1 flex flex-col items-center">
            ...
            {stepTime && (
              <span className="text-xs text-gray-500 mt-1">{stepTime}</span>
            )}
